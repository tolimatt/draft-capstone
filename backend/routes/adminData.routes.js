...
const mapBooking = (booking, usersById, vehiclesById) => {
  ...
  return {
    ...
    blockchainStatus: booking.blockchainTxHash ? "Recorded" : confirmed || booking.status === "completed" ? "Pending" : "Not applicable",
    blockchainRecordedAt: asTimestamp(booking.blockchainRecordedAt),
  };
};
    ...
    try {
      ...
      const [users, vehicles, documents, bookings] = await Promise.all([
        database.collection("users").find({}, {
          projection: { name: 1, email: 1, phone: 1, role: 1, isVerified: 1, kycStatus: 1, createdAt: 1 },
        ...
        database.collection("bookings").find({}, {
          projection: { vehicle: 1, renter: 1, owner: 1, pickupAt: 1, returnAt: 1, status: 1, driverSelected: 1, reviewRating: 1, reviewComment: 1, reviewCreatedAt: 1, blockchainTxHash: 1, blockchainRecordedAt: 1, createdAt: 1 },
        }).sort({ createdAt: -1 }).toArray(),
      ]);
