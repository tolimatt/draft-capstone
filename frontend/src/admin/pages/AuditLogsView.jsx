...
import { ChevronLeft, ChevronRight, LoaderCircle, RefreshCw, ScrollText, ShieldAlert } from "lucide-react";
import { adminDataApi } from "../adminDataApi";
import { EmptyState, FilterSelect, SearchField, StatusBadge } from "../components/AdminUI";
...
export default function AuditLogsView({ initialOutcome = "all" }) {
  ...
  const [payload, setPayload] = useState({ logs: [], actions: [], page: { total: 0, hasMore: false } });
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        {logs.length ? <div className="overflow-x-auto"><table className="w-full min-w-[1050px] border-collapse"><caption className="sr-only">Super Admin audit history</caption><thead className="bg-slate-50"><tr>{["Time", "Action", "Outcome", "Target", "Summary", "Reason", "IP address"].map((heading) => <th key={heading} className="border-b border-slate-200 px-4 py-3 text-left text-[11px] font-bold uppercase tracking-[0.1em] text-slate-600">{heading}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{logs.map((log) => <tr key={log.id} className="align-top hover:bg-blue-50/30"><td className="whitespace-nowrap px-4 py-3 text-xs text-slate-600">{formatTimestamp(log.createdAt)}</td><td className="whitespace-nowrap px-4 py-3 font-mono text-xs font-semibold text-slate-800">{log.action}</td><td className="px-4 py-3"><StatusBadge value={log.outcome === "success" ? "Verified" : "Rejected"} /></td><td className="px-4 py-3 text-xs text-slate-700">{log.targetLabel || "—"}</td><td className="max-w-sm px-4 py-3 text-xs leading-5 text-slate-700">{log.summary}</td><td className="max-w-xs px-4 py-3 text-xs leading-5 text-slate-600">{log.reason || "—"}</td><td className="whitespace-nowrap px-4 py-3 font-mono text-xs text-slate-500">{log.ip || "—"}</td></tr>)}</tbody></table></div> : null}
