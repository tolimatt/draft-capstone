...
import { AlertTriangle, FileSearch, LoaderCircle, RefreshCw, ShieldAlert, X } from "lucide-react";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-end">
        ...
        <button type="button" onClick={() => void load()} disabled={loading} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-blue-600 px-4 text-sm font-semibold text-blue-700 hover:bg-blue-50 disabled:opacity-50"><RefreshCw size={17} className={loading ? "animate-spin" : ""} />Refresh</button>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        {loading ? <div className="flex min-h-72 items-center justify-center"><LoaderCircle size={30} className="animate-spin text-blue-600" /></div> : null}
        {!loading && !reports.length ? <div className="px-6 py-16 text-center"><FileSearch size={34} className="mx-auto text-slate-300" /><h2 className="mt-3 font-bold text-slate-900">No matching reports</h2><p className="mt-1 text-sm text-slate-500">Try another status or priority filter.</p></div> : null}
        {!loading && reports.length ? <div className="divide-y divide-slate-200">{reports.map((report) => <button key={report._id} type="button" onClick={() => { setSelected(report); setNotice(""); }} className="grid w-full gap-3 p-4 text-left hover:bg-slate-50 sm:grid-cols-[1fr_180px_120px] sm:items-center"><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[0.1em] text-blue-600">{report.caseReference}</p><p className="mt-1 truncate font-semibold text-slate-950">{titleCase(report.category)}</p><p className="mt-1 truncate text-xs text-slate-500">{report.reporter?.name} reported {report.reportedUser?.name} · Booking #{String(report.booking?._id || report.booking || "").slice(-6).toUpperCase()}</p></div><span className="text-sm font-medium text-slate-600">{new Date(report.createdAt).toLocaleString()}</span><div className="flex gap-2 sm:justify-end"><Badge value={report.priority} /><Badge value={report.status} /></div></button>)}</div> : null}
      </div>
      {selected ? <CaseDialog report={selected} decision={decision} setDecision={setDecision} notice={notice} saving={saving} onClose={() => setSelected(null)} onUpdate={updateCase} onSubmitDecision={submitDecision} /> : null}
    </div>
  );
}
